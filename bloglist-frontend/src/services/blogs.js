...
const create = async (newObject) => {
  ...
  // ✅ If newObject is FormData, send with multipart headers
  if (newObject instanceof FormData) {
    response = await axios.post(baseUrl, newObject, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })
  } else {
    // Otherwise, send as JSON (fallback)
    response = await axios.post(baseUrl, newObject)
  }
...
const getByTag = async (tag) => {
  const response = await axios.get(`/api/blogs/tags/${encodeURIComponent(tag)}`)
  return response.data
}
